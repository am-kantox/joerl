...
### Паттерн отложенного ответа (Deferred Reply)

joerl поддерживает паттерн отложенного ответа из Erlang с `{noreply, State}` и `gen_server:reply/2`. Это позволяет GenServer'у принять запрос, вернуть управление для обработки других сообщений и ответить позже, когда работа завершится:
...
impl GenServer for JobProcessor {
    async fn handle_call(
        ...
        match job {
            ...
            Job::Slow(data) => {
                // Откладываем ответ для асинхронной работы
                ...
                // Возвращаемся немедленно, не отвечая
...
- Сервер остается отзывчивым во время долгих операций
- Поддерживается несколько одновременных отложенных вызовов
- Вызывающий ждет прозрачно (не знает, что ответ отложен)
- Полностью соответствует семантике Erlang

Полный пример в `examples/gen_server_deferred_reply.rs`.

### Селективный прием (Selective Receive)

Как `receive` с паттерн-матчингом в Erlang, joerl предоставляет селективный прием для ожидания конкретных сообщений, оставляя остальные в mailbox:
...
impl Actor for RpcClient {
    async fn handle_message(&mut self, msg: Message, ctx: &mut ActorContext) {
        if let Some(request_id) = msg.downcast_ref::<u64>() {
            ...
            // Ждем конкретный ответ с совпадающим ID
            ...
            match response {
                Some(resp) => println!("Получен ответ: {:?}", resp),
                None => println!("Таймаут ожидания ответа"),
            }
        }
    }
}
...
**Доступные методы:**
- `ctx.receive(predicate)` - Ждать бесконечно подходящего сообщения
- `ctx.receive_timeout(predicate, timeout)` - Ждать с таймаутом
- `ctx.try_receive(predicate)` - Неблокирующая проверка

**Как это работает:**
- Неподходящие сообщения сохраняются в очередь pending
- Очередь pending проверяется первой при последующих receive
- Сохраняется порядок сообщений через VecDeque
- Идеально для RPC, correlation ID, конечных автоматов

Полный пример RPC с correlation ID в `examples/actor_selective_receive.rs`.
...
| Сообщения остаются в mailbox, если не подходят | То же самое - pending queue их сохраняет |
...
1. **Учиться на примерах**: Смотрите директорию [`examples/`](joerl/examples/)
   ...
   - `gen_server_deferred_reply.rs` — Отложенный ответ с асинхронной работой
   - `actor_selective_receive.rs` — RPC с correlation ID
